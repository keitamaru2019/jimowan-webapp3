...
window.JIMOWAN_APP_VERSION = 'v11-locality-filter';
window.addEventListener('error', function(e) {
  const map = document.getElementById('map');
  if (map && !map.textContent.trim()) {
    map.innerHTML = '<div class="stable-map-empty"><strong>実地図タイル版 v11</strong><br>表示処理でエラーが発生しました。Ctrl+F5で再読み込みしてください。</div>';
  }
});
const state = {
  wonderlands: [],
...
function buildImageSearchUrl(spot, wonderland) {
...
function buildWonderlandPhotoUrl(wonderland) {
  ...
  const spots = (w.spots || []).map((spot, index) => `
    <article class="spot-card">
      ...
      <div class="spot-links">
        <a class="mini-link" href="${escapeHtml(buildImageSearchUrl(spot, w))}" target="_blank" rel="noopener">写真を見る</a>
        <a class="mini-link" href="${escapeHtml(buildMapSearchUrl(spot, w))}" target="_blank" rel="noopener">地図で見る</a>
      </div>
    </article>
  `).join('');
    ...
    <div class="detail-hero">
      <div>
        ...
        <div class="photo-link-panel">
          <p class="photo-link-title">場所がわかる写真リンク</p>
          <div class="spot-links top-links">
            <a class="mini-link" href="${escapeHtml(buildWonderlandPhotoUrl(w))}" target="_blank" rel="noopener">このワンダーランドの写真を見る</a>
          </div>
          <small>各スポット欄にも、写真リンクと地図リンクを付けています。</small>
        </div>
...
function renderActualTileWonderlandMap(w, spots) {
  ...
  if (!spots.length) {
    return '<div class="stable-map-empty"><strong>実地図タイル版 v11</strong><br>緯度経度が登録されたスポットがありません。登録画面で各スポットの緯度・経度を入れてください。</div>';
  }
  ...
  return `
    <div class="actual-map-canvas" role="img" aria-label="${escapeHtml(w.title || 'Wonderland Map')}の実地図マップ">
      ...
      ${segments}
      ${markers}
      <div class="actual-map-version">実地図タイル版 v11</div>
...
function renderMap() {
  ...
  $('#timetableBody').innerHTML = (w.spots || []).map((spot, index) => `
    <tr>
      ...
      <td><div class="map-link-stack"><a class="mini-link" href="${escapeHtml(buildImageSearchUrl(spot, w))}" target="_blank" rel="noopener">写真</a><a class="mini-link" href="${escapeHtml(buildMapSearchUrl(spot, w))}" target="_blank" rel="noopener">地図</a></div></td>
    </tr>
  `).join('');
  ...
  mapEl.innerHTML = mapHtml || '<div class="stable-map-empty"><strong>実地図タイル版 v11</strong><br>マップ生成結果が空です。</div>';
  if (!mapEl.textContent.trim()) {
    mapEl.innerHTML = '<div class="stable-map-empty"><strong>実地図タイル版 v11</strong><br>マップを再描画しました。</div>';
  }
}
