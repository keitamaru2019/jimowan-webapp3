...
window.JIMOWAN_APP_VERSION = 'v18-map-story-header';
window.addEventListener('error', function(e) {
  const map = document.getElementById('map');
  if (map && !map.textContent.trim()) {
    map.innerHTML = '<div class="stable-map-empty"><strong>練馬グリーン版 v18</strong><br>表示処理でエラーが発生しました。Ctrl+F5で再読み込みしてください。</div>';
  }
});
const state = {
  wonderlands: [],
...
function renderActualTileWonderlandMap(w, spots) {
  ...
  if (!spots.length) {
    return '<div class="stable-map-empty"><strong>練馬グリーン版 v18</strong><br>緯度経度が登録されたスポットがありません。登録画面で各スポットの緯度・経度を入れてください。</div>';
  }
  ...
  return `
    <div class="actual-map-canvas" role="img" aria-label="${escapeHtml(localText(w.title || 'Wonderland Map'))}${isEnglish() ? ' actual map' : 'の実地図マップ'}">
      ...
      ${segments}
      ${markers}
      <div class="actual-map-version">練馬グリーン版 v18</div>
...
function renderMap() {
  ...
  const photoUrl = w?.symbolPhotoUrl || w?.photoUrl || w?.heroImage || w?.coverImage || '';
  const photoLink = buildWonderlandMapPhotoUrl(w);
  const photoVisual = photoUrl
    ? `<img src="${escapeHtml(photoUrl)}" alt="${escapeHtml(localText(w.title || 'Wonderland'))}の象徴写真">`
    : `<div class="map-header-symbol map-header-symbol-${symbolicPhotoClass(w)}"><span>${escapeHtml(symbolicPhotoLabel(w))}</span><small>${ui('mapPhoto')}</small></div>`;
  $('#mapTitleBlock').innerHTML = `
    ...
    <a class="map-header-photo" href="${escapeHtml(photoLink)}" target="_blank" rel="noopener" aria-label="${escapeHtml(localText(w.title || 'Wonderland'))}の地図と写真を開く">
      ${photoVisual}
      <strong>${ui('mapPhoto')}</strong>
    </a>
  ...
  mapEl.innerHTML = mapHtml || `<div class="stable-map-empty"><strong>練馬グリーン版 v18</strong><br>${isEnglish() ? 'Map generation returned no result.' : 'マップ生成結果が空です。'}</div>`;
  if (!mapEl.textContent.trim()) {
    mapEl.innerHTML = `<div class="stable-map-empty"><strong>練馬グリーン版 v18</strong><br>${isEnglish() ? 'The map was redrawn.' : 'マップを再描画しました。'}</div>`;
  }
}
