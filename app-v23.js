...
window.JIMOWAN_APP_VERSION = 'v23-ranking-jump-no-loop';
window.addEventListener('error', function(e) {
  const map = document.getElementById('map');
  if (map && !map.textContent.trim()) {
    map.innerHTML = '<div class="stable-map-empty"><strong>練馬グリーン版 v23</strong><br>表示処理でエラーが発生しました。Ctrl+F5で再読み込みしてください。</div>';
  }
});
const state = {
  wonderlands: [],
...
const UI = {
  ja: {
    ...
    entryEyebrow: 'Wonderland Search', entryTitle: '今日はどんなワンダーランドに行こうかな？', entryPlaceholder: '例：親子で、夜に一杯、疲れている、昭和レトロ、練馬の畑', entryButton: '探す', entryHint: '左の区・市町で地域を絞ってから検索すると、その地域の意外なワンダーランドを提案します。',
  ...
  en: {
    ...
    entryEyebrow: 'Wonderland Search', entryTitle: 'What kind of Wonderland shall we visit today?', entryPlaceholder: 'Try: family, a quiet night drink, feeling tired, retro town, Nerima farms', entryButton: 'Search', entryHint: 'Choose a ward/city first, then search by mood or purpose to find unexpected local Wonderlands.',
...
function renderActualTileWonderlandMap(w, spots) {
  ...
  if (!spots.length) {
    return '<div class="stable-map-empty"><strong>練馬グリーン版 v23</strong><br>緯度経度が登録されたスポットがありません。登録画面で各スポットの緯度・経度を入れてください。</div>';
  }
  ...
  return `
    <div class="actual-map-canvas" role="img" aria-label="${escapeHtml(localText(w.title || 'Wonderland Map'))}${isEnglish() ? ' actual map' : 'の実地図マップ'}">
      ...
      ${segments}
      ${markers}
      <div class="actual-map-version">練馬グリーン版 v23</div>
...
function renderMap() {
  ...
  mapEl.innerHTML = mapHtml || `<div class="stable-map-empty"><strong>練馬グリーン版 v23</strong><br>${isEnglish() ? 'Map generation returned no result.' : 'マップ生成結果が空です。'}</div>`;
  if (!mapEl.textContent.trim()) {
    mapEl.innerHTML = `<div class="stable-map-empty"><strong>練馬グリーン版 v23</strong><br>${isEnglish() ? 'The map was redrawn.' : 'マップを再描画しました。'}</div>`;
  }
}
