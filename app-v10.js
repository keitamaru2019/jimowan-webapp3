...
window.JIMOWAN_APP_VERSION = 'v10-photo-links';
window.addEventListener('error', function(e) {
  const map = document.getElementById('map');
  if (map && !map.textContent.trim()) {
    map.innerHTML = '<div class="stable-map-empty"><strong>実地図タイル版 v10</strong><br>表示処理でエラーが発生しました。Ctrl+F5で再読み込みしてください。</div>';
  }
});
const state = {
  wonderlands: [],
...
function renderActualTileWonderlandMap(w, spots) {
  ...
  if (!spots.length) {
    return '<div class="stable-map-empty"><strong>実地図タイル版 v10</strong><br>緯度経度が登録されたスポットがありません。登録画面で各スポットの緯度・経度を入れてください。</div>';
  }
  ...
  return `
    <div class="actual-map-canvas" role="img" aria-label="${escapeHtml(w.title || 'Wonderland Map')}の実地図マップ">
      ...
      ${segments}
      ${markers}
      <div class="actual-map-version">実地図タイル版 v10</div>
...
function renderMap() {
  ...
  mapEl.innerHTML = mapHtml || '<div class="stable-map-empty"><strong>実地図タイル版 v10</strong><br>マップ生成結果が空です。</div>';
  if (!mapEl.textContent.trim()) {
    mapEl.innerHTML = '<div class="stable-map-empty"><strong>実地図タイル版 v10</strong><br>マップを再描画しました。</div>';
  }
}
