...
window.JIMOWAN_APP_VERSION = 'v26-jimowan-name-ready';
window.addEventListener('error', function(e) {
  const map = document.getElementById('map');
  if (map && !map.textContent.trim()) {
    map.innerHTML = '<div class="stable-map-empty"><strong>仕上げ版 v26</strong><br>表示処理でエラーが発生しました。Ctrl+F5で再読み込みしてください。</div>';
  }
});
const state = {
  wonderlands: [],
...
const UI = {
  ja: {
    ...
    entryEyebrow: 'Wonderland Search', entryTitle: '今日はどんなワンダーランドに行こうかな？', entryPlaceholder: '例：親子で、夜に一杯、疲れている、昭和レトロ、練馬の畑', entryButton: '探す', entryHint: '左の区・市町で地域を絞ってから検索すると、その地域の意外なワンダーランドを提案します。', openingEyebrow: '知られていないことに価値がある', openingTitle: '遊園地がなくてもいい。<br>つなげば、そこは夢の場所になる。', openingLead: '地元ファンしか知らない、コアなドキドキを体験しよう。', whatIsJimowan: 'ジモワンって何？', openingRanking: 'ランキングを見る',
    portalTitle: '今回の地元ワンダーランド＝ジモワン', portalDesc: '登録・投票・マップ生成ができるMVPです。データはこのサーバー内のJSONに保存されます。', localityLabel: '区・市町で選ぶ', allAreas: 'すべての区・市町', allAreaShort: '全地域',
    tabDiscover: '探す', tabRanking: 'ランキング', tabRegister: 'ワンダーランド登録', tabMap: 'ワンダーランドマップ作成', tabKml: 'KML取込', rankingFormulaTitle: 'ランキング式', discoverTitle: 'ワンダーランドを探す', freeSearchLabel: '自由検索欄', freeSearchPlaceholder: '例：疲れてる、親子で、夜に一杯、昭和レトロ、練馬の畑、雨でも楽しめる', freeSearchButton: '意外なワンダーランドを探す',
    rankingTitle: 'ワンダーランドランキング', voteTitle: '投票して順位を更新', voteButton: '投票する', registerTitle: 'ワンダーランドを登録する', spotRegisterTitle: 'スポット登録', registerButton: '登録してランキングへ反映', fillExample: '入力例を入れる', mapTitle: 'ワンダーランドマップ作成', printMap: 'PDF保存 / 印刷',
    rankSuffix: '位', spots: 'スポット', current: '現在', duration: '所要時間', totalDistance: '総距離', totalScore: '総合点', voters: '投票者', unknownAvg: '知らなかった平均', surpriseAvg: '予想外平均', tryAvg: '初体験平均', totalAvg: '合計平均', unknownP: '知らなかったP', surpriseP: '予想外P', tryP: '初体験P', totalP: '合計点', creator: '情報源', jimowan: 'ワンダーランド', mapPhoto: '地図/写真', stay: '滞在', minutes: '分',
    pdf: '原本PDFを開く', sourcePage: '参照ページを開く', sourceNote: '出典メモ', locationMapPhotoTitle: '場所がわかる地図/写真リンク', mapPhotoHint: '各スポット欄の「地図/写真」から、Googleマップ上で場所と写真を確認できます。', chooseThis: 'このワンダーランドを見る', noWonderlands: 'まだワンダーランドが登録されていません。', kmlTitle: '東京ウォーキングマップ KML取込', kmlSubtitle: 'KMLをワンダーランド形式に変換して、区・市町ランキングとマップに追加します。', kmlFileLabel: 'KMLファイルを選択', kmlLimitLabel: '取り込み上限件数', kmlSourceLabel: '取込元メモ', kmlButton: 'KMLを取り込む', kmlHint: 'KML内のコース名、Point、LineStringを読み取り、ワンダーランド用のタイトル・スポット・ルート線・仮ランキング点に変換します。'
  },
  en: {
    langButton: '日本語', navLp: 'LP', navRanking: 'Ranking', navRegister: 'Register', navMap: 'Map', navKml: 'KML Import', navLogout: 'Logout',
    entryEyebrow: 'Wonderland Search', entryTitle: 'What kind of Wonderland shall we visit today?', entryPlaceholder: 'Try: family, a quiet night drink, feeling tired, retro town, Nerima farms', entryButton: 'Search', entryHint: 'Choose a ward/city first, then search by mood or purpose to find unexpected local Wonderlands.', openingEyebrow: 'Hidden places have hidden value', openingTitle: 'You do not need an amusement park.<br>Connect what is already there, and it becomes a dream place.', openingLead: 'Experience the hidden thrills only local fans know.', whatIsJimowan: 'What is Jimowan?', openingRanking: 'View ranking',
    portalTitle: 'Today’s Local Wonderland = Jimowan', portalDesc: 'A working MVP for registration, voting, ranking, and map generation. Data is stored as JSON on this server.', localityLabel: 'Choose ward/city', allAreas: 'All wards/cities', allAreaShort: 'All areas',
    tabDiscover: 'Discover', tabRanking: 'Ranking', tabRegister: 'Register Wonderland', tabMap: 'Wonderland Map Maker', tabKml: 'KML Import', rankingFormulaTitle: 'Ranking formula', discoverTitle: 'Find a Wonderland', freeSearchLabel: 'Free search', freeSearchPlaceholder: 'Try: tired, family, a drink at night, retro town, Nerima farms, rainy day', freeSearchButton: 'Find an unexpected Wonderland',
    rankingTitle: 'Wonderland Ranking', voteTitle: 'Vote and update the ranking', voteButton: 'Vote', registerTitle: 'Register a Wonderland', spotRegisterTitle: 'Register spots', registerButton: 'Register and update ranking', fillExample: 'Fill sample', mapTitle: 'Wonderland Map Maker', printMap: 'Save PDF / Print',
    rankSuffix: '', spots: 'spots', current: 'Current', duration: 'Duration', totalDistance: 'Distance', totalScore: 'Total score', voters: 'Voters', unknownAvg: 'Unknown avg.', surpriseAvg: 'Surprise avg.', tryAvg: 'Want-to-try avg.', totalAvg: 'Total avg.', unknownP: 'Unknown P', surpriseP: 'Surprise P', tryP: 'Want-to-try P', totalP: 'Total', creator: 'Source', jimowan: 'Wonderland', mapPhoto: 'Map/Photos', stay: 'Stay', minutes: 'min',
    pdf: 'Open source PDF', sourcePage: 'Open reference page', sourceNote: 'Source note', locationMapPhotoTitle: 'Map/Photo links for each place', mapPhotoHint: 'Use “Map/Photos” in each spot card to confirm the place and photos on Google Maps.', chooseThis: 'View this Wonderland', noWonderlands: 'No Wonderlands registered yet.', kmlTitle: 'Import TOKYO WALKING MAP KML', kmlSubtitle: 'Convert KML into Wonderland entries and add them to ward/city rankings and maps.', kmlFileLabel: 'Choose KML file', kmlLimitLabel: 'Import limit', kmlSourceLabel: 'Source memo', kmlButton: 'Import KML', kmlHint: 'Reads course names, Points, and LineStrings from KML, then creates titles, spots, route lines, and temporary ranking points.'
  }
};
...
function firstRepresentativeSpot(wonderland) {
  const spots = wonderland?.spots || [];
  return spots.find((s) => s && s.name && Number.isFinite(Number(s.lat)) && Number.isFinite(Number(s.lng))) || spots.find((s) => s && s.name) || null;
}

function buildWonderlandMapPhotoUrl(wonderland) {
  const spot = firstRepresentativeSpot(wonderland);
  if (spot) return buildMapSearchUrl(spot, wonderland);
  const q = [wonderland?.title || '', wonderland?.area || '', '写真'].filter(Boolean).join(' ');
  return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(q)}`;
}


function mapHeaderPhotoFallback(wonderland) {
  return `<div class="map-header-symbol map-header-symbol-${symbolicPhotoClass(wonderland)}"><span>${escapeHtml(symbolicPhotoLabel(wonderland))}</span><small>${ui('mapPhoto')}</small></div>`;
}

async function fetchRepresentativePhoto(wonderland) {
  if (!wonderland?.id) return null;
  if (state.photoCache[wonderland.id] !== undefined) return state.photoCache[wonderland.id];
  const existing = wonderland?.symbolPhotoUrl || wonderland?.photoUrl || wonderland?.heroImage || wonderland?.coverImage || '';
  if (existing) {
    const photo = { imageUrl: existing, pageUrl: wonderland?.symbolPhotoPageUrl || wonderland?.sourceUrl || '', sourceName: wonderland?.symbolPhotoSource || '' };
    state.photoCache[wonderland.id] = photo;
    return photo;
  }
  try {
    const result = await api(`/api/wonderland-photo?id=${encodeURIComponent(wonderland.id)}`);
    const photo = result?.photo || null;
    state.photoCache[wonderland.id] = photo;
    if (photo?.imageUrl) {
      wonderland.symbolPhotoUrl = photo.imageUrl;
      wonderland.symbolPhotoPageUrl = photo.pageUrl || '';
      wonderland.symbolPhotoSource = photo.sourceName || '';
    }
    return photo;
  } catch (error) {
    state.photoCache[wonderland.id] = null;
    return null;
  }
}

function renderMapHeaderPhoto(container, wonderland, photo) {
  if (!container) return;
  const link = (photo && (photo.pageUrl || photo.imageUrl)) || buildWonderlandMapPhotoUrl(wonderland);
  const photoUrl = photo?.imageUrl || wonderland?.symbolPhotoUrl || wonderland?.photoUrl || wonderland?.heroImage || wonderland?.coverImage || '';
  container.href = link;
  const visual = container.querySelector('.map-header-photo-visual');
  if (!visual) return;
  const source = container.querySelector('.map-photo-source-holder');
  if (photo?.sourceName && source) source.innerHTML = `<small class="map-photo-source">${escapeHtml(photo.sourceName)}</small>`;
  if (photoUrl) {
    const fallback = escapeHtml(mapHeaderPhotoFallback(wonderland)).replaceAll("'", "&#039;");
    visual.innerHTML = `<img src="${escapeHtml(photoUrl)}" alt="${escapeHtml(localText(wonderland.title || 'Wonderland'))}${isEnglish() ? ' representative photo' : 'の象徴写真'}" onerror="this.closest('.map-header-photo-visual').innerHTML='${fallback}';">`;
  } else {
    visual.innerHTML = mapHeaderPhotoFallback(wonderland);
  }
}

function compactDescription(wonderland) {
  const source = wonderland?.description || wonderland?.catchCopy || firstRepresentativeSpot(wonderland)?.notes || '';
  const text = localText(source).replace(/\s+/g, ' ').trim();
  const max = isEnglish() ? 185 : 105;
  return text.length > max ? `${text.slice(0, max)}…` : text;
}

function shortWonderlandType(wonderland) {
  const source = localText(wonderland?.catchCopy || wonderland?.description || firstRepresentativeSpot(wonderland)?.notes || wonderland?.title || '');
  const normalized = source
    ...
    .replace(/\s+/g, ' ')
    .trim();
  const tags = (wonderland?.tags || []).map((t) => localText(t)).filter(Boolean);
  let text = '';
  if (/農|畑|野菜|大根/.test(source)) text = '畑をめぐる発見';
  else if (/銭湯|温泉|湯/.test(source)) text = '湯と街の寄り道';
  else if (/公園|森|自然|池|庭園/.test(source)) text = '自然に出会う散歩';
  else if (/酒|バー|ジャズ|夜/.test(source)) text = '夜を楽しむ街歩き';
  else if (/歴史|文化|寺|神社|古墳|資料館|文学/.test(source)) text = '歴史文化さんぽ';
  else if (/喫茶|珈琲|カフェ|グルメ|食|餃子|たい焼き|麺/.test(source)) text = '食を楽しむ寄り道';
  else if (/親子|子ども|キッズ|乗り物/.test(source)) text = '親子で発見散歩';
  else if (/レトロ|昭和|路地|商店街/.test(source)) text = 'レトロ街歩き';
  else if (tags.length) text = `${tags[0]}の発見散歩`;
  else text = normalized || localText(wonderland?.title || '');
  const limit = isEnglish() ? 42 : 15;
  return text.length > limit ? `${text.slice(0, limit - 1)}…` : text;
}

function symbolicPhotoClass(wonderland) {
  const text = rawWonderlandText(wonderland);
  if (/温泉|銭湯|湯/.test(text)) return 'bath';
  if (/公園|森|自然|池|庭園/.test(text)) return 'park';
  if (/酒|バー|ジャズ|夜|居酒屋/.test(text)) return 'night';
  if (/畑|農|野菜|大根/.test(text)) return 'farm';
  if (/古墳|歴史|文学|文化|寺|神社|資料館/.test(text)) return 'culture';
  if (/餃子|たい焼き|麺|グルメ|食|喫茶|珈琲/.test(text)) return 'food';
  return 'local';
}

function symbolicPhotoLabel(wonderland) {
  const spot = firstRepresentativeSpot(wonderland);
  return localText((spot && spot.name) || wonderland?.area || wonderland?.title || 'Wonderland');
}

function renderMapIntro(wonderland) {
  const photoUrl = wonderland?.symbolPhotoUrl || wonderland?.photoUrl || wonderland?.heroImage || wonderland?.coverImage || '';
  const linkUrl = buildWonderlandMapPhotoUrl(wonderland);
  const meta = [
    localText(wonderland?.area || ''),
    wonderland?.duration ? `${isEnglish() ? 'Duration' : '所要時間'} ${localText(wonderland.duration)}` : '',
    `${Number(wonderland?.totalDistanceKm || 0).toFixed(2)}km`,
    `${(wonderland?.spots || []).length}${isEnglish() ? ' spots' : 'スポット'}`
  ].filter(Boolean);
  const visual = photoUrl
    ? `<img src="${escapeHtml(photoUrl)}" alt="${escapeHtml(localText(wonderland.title || 'Wonderland'))}の象徴写真">`
    : `<div class="symbolic-photo symbolic-photo-${symbolicPhotoClass(wonderland)}"><span>${escapeHtml(symbolicPhotoLabel(wonderland))}</span><small>${isEnglish() ? 'Open Map/Photos' : '地図/写真を開く'}</small></div>`;
  return `
    <div class="map-intro-copy">
      <p class="eyebrow">${isEnglish() ? 'Wonderland Story' : 'ワンダーランド紹介'}</p>
      <h4>${escapeHtml(localText(wonderland.catchCopy || wonderland.title || ''))}</h4>
      <p>${escapeHtml(compactDescription(wonderland))}</p>
      <div class="map-intro-meta">${meta.map((m) => `<span>${escapeHtml(m)}</span>`).join('')}</div>
    </div>
    <a class="map-symbol-photo" href="${escapeHtml(linkUrl)}" target="_blank" rel="noopener" aria-label="${escapeHtml(localText(wonderland.title || 'Wonderland'))}の地図と写真を開く">
      ${visual}
      <strong>${isEnglish() ? 'Map/Photos' : '地図/写真'}</strong>
    </a>
  `;
}
...
const LOCALITY_PREFECTURES = {
  ...
  // Other sample areas
  '伊勢原市':'神奈川県','藤沢市':'神奈川県','川崎市宮前区':'神奈川県','掛川市':'静岡県','丸亀市':'香川県','宇都宮市':'栃木県','茂木町':'栃木県','真岡市':'栃木県','川越市':'埼玉県','朝霞市':'埼玉県','北九州市八幡西区':'福岡県','堺市北区':'大阪府','札幌市中央区':'北海道','札幌市北区':'北海道','高浜市':'愛知県','船橋市':'千葉県'
};
...
function getLocalityOptions() {
  const order = ['練馬区', '宇都宮市', '大田区', '江戸川区', '目黒区', '西東京市', '国立市', '調布市', '中央区', '世田谷区', '川越市', '朝霞市', '船橋市', '北九州市八幡西区', '堺市北区', '札幌市中央区', '札幌市北区', '高浜市', '三鷹市', '武蔵野市', '伊勢原市', '藤沢市', '川崎市宮前区', '掛川市', '丸亀市', '茂木町', '真岡市', '豊島区'];
  const set = new Set();
...
function renderSearchResults() {
  ...
  $$('.choose-recommend', target).forEach((button) => {
    button.addEventListener('click', () => {
      jumpToRankingForWonderland(button.dataset.id);
      showNotice(isEnglish() ? 'Jumped to this Wonderland in the ranking.' : 'ランキング内の該当ワンダーランドへ移動しました。');
    });
  });
}
...
function jumpToRankingForWonderland(id) {
  if (!id) return;
  state.currentId = id;
  const target = state.wonderlands.find((w) => w.id === id);
  if (target) {
    const locs = getWonderlandLocalities(target);
    if (locs.length) {
      state.selectedLocality = locs[0];
      localStorage.setItem('jimowanLocality', state.selectedLocality);
      const localitySelect = $('#localitySelect');
      if (localitySelect) localitySelect.value = state.selectedLocality;
    }
  }
  ['discoverSelect','mapSelect','voteWonderland'].forEach((selectId) => {
  ...
  renderRanking();
  window.setTimeout(() => {
    const row = document.querySelector(`.ranking-row[data-id="${CSS.escape(id)}"]`);
    if (row) {
      row.scrollIntoView({ behavior: 'smooth', block: 'center' });
      row.classList.add('ranking-row-highlight');
      window.setTimeout(() => row.classList.remove('ranking-row-highlight'), 2200);
    }
  }, 80);
}
...
function renderRanking() {
  ...
  if (headers.length >= 9) {
    const labels = isEnglish()
      ? ['Rank','Wonderland','Source','What kind?','Unknown P','Surprise P','Want-to-try P','Total','Voters']
      : ['順位','ワンダーランド','情報源','どんなワンダーランド？','知らなかったP','予想外P','初体験P','合計点','投票者'];
  ...
  body.innerHTML = list.length ? list.map((w, index) => `
    <tr class="ranking-row" data-id="${escapeHtml(w.id)}" tabindex="0" title="${isEnglish() ? 'Open in Wonderland Map Maker' : 'ワンダーランドマップ作成で開く'}">
      <td class="rank-cell">${index === 0 ? '<span class="rank-1">1</span>' : index + 1}</td>
      <td class="ranking-wonderland-cell"><strong>${escapeHtml(localText(w.title))}</strong><br><span class="muted">${escapeHtml(localText(w.area))} / ${label}</span><br><button type="button" class="link-button open-map-from-ranking" data-id="${escapeHtml(w.id)}">${isEnglish() ? 'Open map' : 'マップを見る'}</button></td>
      <td>${escapeHtml(w.creator || '')}</td>
      <td><strong>${escapeHtml(shortWonderlandType(w))}</strong></td>
  ...
  `).join('') : `<tr><td colspan="9" class="empty-row">${isEnglish() ? 'No matching Wonderlands. Try changing the filter.' : '該当するワンダーランドがありません。検索条件を変えてください。'}</td></tr>`;

  $$('.ranking-row', body).forEach((row) => {
    const open = () => {
      ...
      setActiveTab('map');
      renderMap();
      document.getElementById('tab-map')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    };
    row.addEventListener('click', (event) => {
      if (event.target.closest('button, a, select, input')) {
        if (event.target.closest('.open-map-from-ranking')) open();
        return;
      }
      open();
    });
...
function renderActualTileWonderlandMap(w, spots) {
  ...
  if (!spots.length) {
    return '<div class="stable-map-empty"><strong>仕上げ版 v26</strong><br>緯度経度が登録されたスポットがありません。登録画面で各スポットの緯度・経度を入れてください。</div>';
  }
  ...
  return `
    <div class="actual-map-canvas" role="img" aria-label="${escapeHtml(localText(w.title || 'Wonderland Map'))}${isEnglish() ? ' actual map' : 'の実地図マップ'}">
      ...
      ${segments}
      ${markers}
      <div class="actual-map-version">仕上げ版 v26</div>
...
function renderMap() {
  ...
  if (!w) return;
  state.currentId = w.id;
  const storyText = compactDescription(w);
  const initialPhotoUrl = w?.symbolPhotoUrl || w?.photoUrl || w?.heroImage || w?.coverImage || '';
  const initialFallback = mapHeaderPhotoFallback(w);
  const initialPhotoVisual = initialPhotoUrl
    ? `<img src="${escapeHtml(initialPhotoUrl)}" alt="${escapeHtml(localText(w.title || 'Wonderland'))}${isEnglish() ? ' representative photo' : 'の象徴写真'}" onerror="this.closest('.map-header-photo-visual').innerHTML='${escapeHtml(initialFallback).replaceAll("'", "&#039;")}';">`
    : initialFallback;
  $('#mapTitleBlock').innerHTML = `
    <div class="map-header-title map-header-fixed-box">
      <p class="eyebrow">Wonderland Map</p>
      <h3>${escapeHtml(localText(w.title))}</h3>
      <p>${escapeHtml(localText(w.area))} / ${escapeHtml(w.creator || '')}</p>
    </div>
    <div class="map-header-story map-header-fixed-box" aria-label="${isEnglish() ? 'Wonderland description' : 'ワンダーランド紹介文'}">
      <p class="map-header-label">${isEnglish() ? 'Wonder Point' : 'このワンダーランドの紹介文'}</p>
      <p>${escapeHtml(storyText)}</p>
    </div>
    <a class="map-header-photo map-header-fixed-box" id="mapHeaderPhoto" href="${escapeHtml(buildWonderlandMapPhotoUrl(w))}" target="_blank" rel="noopener" aria-label="${escapeHtml(localText(w.title || 'Wonderland'))}${isEnglish() ? ' representative photo' : 'の象徴写真'}">
      <div class="map-header-photo-visual">${initialPhotoVisual}</div>
      <strong>${isEnglish() ? 'Representative photo' : '象徴写真'}</strong>
      <span class="map-photo-source-holder"></span>
    </a>
    <div class="map-rank map-header-rank map-header-fixed-box"><strong>${rankText(w.rank)}</strong><span>${yenLike(w.ranking.total)}P</span></div>
  `;
  fetchRepresentativePhoto(w).then((photo) => {
    if (state.currentId !== w.id) return;
    renderMapHeaderPhoto(document.getElementById('mapHeaderPhoto'), w, photo);
  });
  const intro = $('#mapIntroBlock');
  ...
  mapEl.innerHTML = mapHtml || `<div class="stable-map-empty"><strong>仕上げ版 v26</strong><br>${isEnglish() ? 'Map generation returned no result.' : 'マップ生成結果が空です。'}</div>`;
  if (!mapEl.textContent.trim()) {
    mapEl.innerHTML = `<div class="stable-map-empty"><strong>仕上げ版 v26</strong><br>${isEnglish() ? 'The map was redrawn.' : 'マップを再描画しました。'}</div>`;
  }
}
