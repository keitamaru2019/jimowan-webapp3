...
window.JIMOWAN_APP_VERSION = 'v19-auto-symbol-photo';
window.addEventListener('error', function(e) {
  const map = document.getElementById('map');
  if (map && !map.textContent.trim()) {
    map.innerHTML = '<div class="stable-map-empty"><strong>練馬グリーン版 v19</strong><br>表示処理でエラーが発生しました。Ctrl+F5で再読み込みしてください。</div>';
  }
});
const state = {
  wonderlands: [],
...
function renderActualTileWonderlandMap(w, spots) {
  ...
  if (!spots.length) {
    return '<div class="stable-map-empty"><strong>練馬グリーン版 v19</strong><br>緯度経度が登録されたスポットがありません。登録画面で各スポットの緯度・経度を入れてください。</div>';
  }
  ...
  return `
    <div class="actual-map-canvas" role="img" aria-label="${escapeHtml(localText(w.title || 'Wonderland Map'))}${isEnglish() ? ' actual map' : 'の実地図マップ'}">
      ...
      ${segments}
      ${markers}
      <div class="actual-map-version">練馬グリーン版 v19</div>
...
function renderMap() {
  ...
  const initialPhotoVisual = initialPhotoUrl
    ? `<img src="${escapeHtml(initialPhotoUrl)}" alt="${escapeHtml(localText(w.title || 'Wonderland'))}${isEnglish() ? ' representative photo' : 'の象徴写真'}">`
    : `<div class="map-header-symbol map-header-symbol-${symbolicPhotoClass(w)}"><span>${escapeHtml(symbolicPhotoLabel(w))}</span><small>${isEnglish() ? 'Searching for a representative photo...' : '説明文に合う写真を探しています...'}</small></div>`;
  $('#mapTitleBlock').innerHTML = `
    ...
    <div class="map-header-story" aria-label="${isEnglish() ? 'Wonderland description' : 'ワンダーランド紹介文'}">
      <p>${escapeHtml(storyText)}</p>
    </div>
    <a class="map-header-photo" id="mapHeaderPhoto" href="${escapeHtml(buildWonderlandMapPhotoUrl(w))}" target="_blank" rel="noopener" aria-label="${escapeHtml(localText(w.title || 'Wonderland'))}${isEnglish() ? ' representative photo' : 'の象徴写真'}">
      ${initialPhotoVisual}
      <strong>${isEnglish() ? 'Representative photo' : '象徴写真'}</strong>
    </a>
    <div class="map-rank map-header-rank"><strong>${rankText(w.rank)}</strong><span>${yenLike(w.ranking.total)}P</span></div>
  `;
  ...
  mapEl.innerHTML = mapHtml || `<div class="stable-map-empty"><strong>練馬グリーン版 v19</strong><br>${isEnglish() ? 'Map generation returned no result.' : 'マップ生成結果が空です。'}</div>`;
  if (!mapEl.textContent.trim()) {
    mapEl.innerHTML = `<div class="stable-map-empty"><strong>練馬グリーン版 v19</strong><br>${isEnglish() ? 'The map was redrawn.' : 'マップを再描画しました。'}</div>`;
  }
}
