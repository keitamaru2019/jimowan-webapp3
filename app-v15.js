...
window.JIMOWAN_APP_VERSION = 'v15-nerima-green';
window.addEventListener('error', function(e) {
  const map = document.getElementById('map');
  if (map && !map.textContent.trim()) {
    map.innerHTML = '<div class="stable-map-empty"><strong>練馬グリーン版 v15</strong><br>表示処理でエラーが発生しました。Ctrl+F5で再読み込みしてください。</div>';
  }
});
const state = {
  wonderlands: [],
...
function renderActualTileWonderlandMap(w, spots) {
  ...
  if (!spots.length) {
    return '<div class="stable-map-empty"><strong>練馬グリーン版 v15</strong><br>緯度経度が登録されたスポットがありません。登録画面で各スポットの緯度・経度を入れてください。</div>';
  }
  ...
  return `
    <div class="actual-map-canvas" role="img" aria-label="${escapeHtml(localText(w.title || 'Wonderland Map'))}${isEnglish() ? ' actual map' : 'の実地図マップ'}">
      ...
      ${segments}
      ${markers}
      <div class="actual-map-version">練馬グリーン版 v15</div>
...
function renderMap() {
  ...
  mapEl.innerHTML = mapHtml || `<div class="stable-map-empty"><strong>練馬グリーン版 v15</strong><br>${isEnglish() ? 'Map generation returned no result.' : 'マップ生成結果が空です。'}</div>`;
  if (!mapEl.textContent.trim()) {
    mapEl.innerHTML = `<div class="stable-map-empty"><strong>練馬グリーン版 v15</strong><br>${isEnglish() ? 'The map was redrawn.' : 'マップを再描画しました。'}</div>`;
  }
}
