...
window.JIMOWAN_APP_VERSION = 'v21-nerima-loop-plan';
window.addEventListener('error', function(e) {
  const map = document.getElementById('map');
  if (map && !map.textContent.trim()) {
    map.innerHTML = '<div class="stable-map-empty"><strong>練馬グリーン版 v21</strong><br>表示処理でエラーが発生しました。Ctrl+F5で再読み込みしてください。</div>';
  }
});
const state = {
  wonderlands: [],
...
const UI = {
  ja: {
    ...
    rankingTitle: 'ジモワンランキング', voteTitle: '投票して順位を更新', voteButton: '投票する', registerTitle: 'ワンダーランドを登録する', spotRegisterTitle: 'スポット登録', registerButton: '登録してランキングへ反映', fillExample: '入力例を入れる', mapTitle: 'ワンダーランドマップ作成', printMap: 'PDF保存 / 印刷',
  ...
  en: {
    ...
    rankingTitle: 'Jimowan Ranking', voteTitle: 'Vote and update the ranking', voteButton: 'Vote', registerTitle: 'Register a Wonderland', spotRegisterTitle: 'Register spots', registerButton: 'Register and update ranking', fillExample: 'Fill sample', mapTitle: 'Wonderland Map Maker', printMap: 'Save PDF / Print',
...
function renderActualTileWonderlandMap(w, spots) {
  ...
  if (!spots.length) {
    return '<div class="stable-map-empty"><strong>練馬グリーン版 v21</strong><br>緯度経度が登録されたスポットがありません。登録画面で各スポットの緯度・経度を入れてください。</div>';
  }
  ...
  return `
    <div class="actual-map-canvas" role="img" aria-label="${escapeHtml(localText(w.title || 'Wonderland Map'))}${isEnglish() ? ' actual map' : 'の実地図マップ'}">
      ...
      ${segments}
      ${markers}
      <div class="actual-map-version">練馬グリーン版 v21</div>
...
function renderMap() {
  ...
  mapEl.innerHTML = mapHtml || `<div class="stable-map-empty"><strong>練馬グリーン版 v21</strong><br>${isEnglish() ? 'Map generation returned no result.' : 'マップ生成結果が空です。'}</div>`;
  if (!mapEl.textContent.trim()) {
    mapEl.innerHTML = `<div class="stable-map-empty"><strong>練馬グリーン版 v21</strong><br>${isEnglish() ? 'The map was redrawn.' : 'マップを再描画しました。'}</div>`;
  }
}
